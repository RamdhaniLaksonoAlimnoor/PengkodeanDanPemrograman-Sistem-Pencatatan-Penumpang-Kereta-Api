...
// Mengambil data dari table siswa dari nik yang tidak sama dengan 0
$siswa = query("SELECT * FROM siswa WHERE nik = $nik")[0];
...
if (isset($_POST['ubah'])) {
    if (ubah($_POST) > 0) {
        ...
        echo "<script>
                alert('Data Penumpang gagal diubah!');
            </script>";
    }
}
...
<body background="img/bg/kereta bg.jpg">
     <!-- Navbar -->
     <nav class="navbar navbar-expand-lg navbar-dark bg-dark text-uppercase">
          <div class="container">
               <a class="navbar-brand" href="index.php">Sistem Admin Data Penumpang Kereta Api</a>
               ...
               <div class="col-md">
                    <form action="" method="post" enctype="multipart/form-data">
                         ...
                         <div class="mb-3">
                              ...
                              <input type="number" class="form-control w-50" id="nik" value="<?= $siswa['nik']; ?>"
                                   name="nik" readonly>
                         </div>
                         <div class="mb-3">
                         ...
                         <div class="mb-3">
                              <label for="kereta" class="form-label">Kereta</label>
                              <select class="form-select w-50" id="kereta" name="kereta">
                                   <option disabled selected value>--------------------------------------------Pilih
                                        kereta--------------------------------------------</option>
                                   <option value="Kaligung" <?php if ($siswa['kereta'] == 'Kaligung') { ?>
                                        selected='' <?php } ?>>Kaligung</option>
                                   <option value="Kamandaka"
                                        <?php if ($siswa['kereta'] == 'Kamandaka') { ?> selected=''
                                        <?php } ?>>Kamandaka</option>
                                   <option value="Joglosemarkerto" <?php if ($siswa['kereta'] == 'Joglosemarkerto') { ?>
                                        selected='' <?php } ?>>Joglosemarkerto</option>
                                   <option value="Argo Bromo Anggrek"
                                        <?php if ($siswa['kereta'] == 'Argo Bromo Anggrek') { ?> selected=''
                                        <?php } ?>>Argo Bromo Anggrek</option>
                                   <option value="Majapahit" <?php if ($siswa['kereta'] == 'Majapahit') { ?>
                                        selected='' <?php } ?>>Majapahit</option>
                                   <option value="Tawang Jaya Premium" <?php if ($siswa['kereta'] == 'Tawang Jaya Premium') { ?> selected=''
                                        <?php } ?>>Tawang Jaya Premium</option>
                              </select>
                         </div>
                         <div class="mb-3">
     ...
     gsap.to('.ubah_data', {
          duration: 2,
          delay: 1,
          text: '<i class="bi bi-pencil-square"></i>Ubah Data Penumpang'
     })
     gsap.from('.navbar', {
          duration: 1,
          y: '-100%',
